...
export default function AdminDashboardPage() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'users' | 'departments' | 'shifts'>('overview')
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col">
      {/* Top Navbar */}
      <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-border bg-card/90 px-4 md:px-8 backdrop-blur-md">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-base font-bold tracking-tight md:text-lg">MES Admin Portal</h1>
              <span className="rounded-full bg-primary/10 px-2 py-0.5 text-[11px] font-semibold text-primary">
                Quản trị Tổ chức
              </span>
            </div>
            <p className="text-xs text-muted-foreground hidden sm:block">
              Hệ thống quản trị cơ cấu phòng ban, ca làm việc và đồng bộ nhóm chat
            </p>
          </div>
        </div>
